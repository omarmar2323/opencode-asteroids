...
- All game logic is in `game.js` (~780 lines). `index.html` loads it directly via `<script>` tag.
...
- Skin system: 5 skins (CLÁSICO, NEÓN, FURIA, REAL, FANTASMA). Select with ← → on title screen, Tab during gameplay. Persisted in localStorage.
